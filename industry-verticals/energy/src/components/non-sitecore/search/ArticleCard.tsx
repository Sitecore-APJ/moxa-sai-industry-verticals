...
const ArticleItemCard = ({ className = '', article }: ArticleItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border relative border transition-shadow hover:shadow-md ${className}`}
      >
        <div className="bg-background-muted h-50 w-full overflow-hidden">
          <Image
            src={validImageUrl}
