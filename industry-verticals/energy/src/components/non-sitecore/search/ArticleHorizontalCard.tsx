...
const ArticleHorizontalItemCard = ({ className = '', article }: ArticleCardItemCardProps) => {
  ...
  return (
    ...
    >
      <ArticleCard.Root
        key={article.id}
        className={`group border-border bg-background relative my-4 flex max-h-52 w-full flex-row flex-nowrap border p-6 transition-shadow hover:shadow-md ${className}`}
      >
        {validImageUrl && (
          <div className="bg-background-muted w-1/4 flex-none overflow-hidden">
            <Image
              src={validImageUrl}
        ...
        <div className="grow flex-col pl-4">
          ...
          <div className="text-foreground bg-background-accent border-accent absolute top-4 right-4 border px-2.5 py-0.5 text-xs font-semibold uppercase">
            {article.type}
          </div>
        </div>
      </ArticleCard.Root>
